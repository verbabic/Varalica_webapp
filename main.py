...
class Room:
    ...
    player_list_reactions: list[dict] = field(default_factory=list)
    active_target_reactions: list[dict] = field(default_factory=list)
    turn_version: int = 0
...
async def create_room(request: NameRequest) -> dict:
    ...
    room_play_mode = normalize_play_mode(request.play_mode)
    room = Room(
        code=room_code,
        used_word_indexes=load_used_word_indexes(room_code),
        play_mode=room_play_mode,
    )
    player = Player(
        id=str(uuid.uuid4()),
        name=name,
    ...
    persist_room(room)
    return {"room_code": room_code, "player_id": player.id, "avatar": player.avatar, "room_play_mode": room.play_mode}
...
async def join_room(room_code: str, request: NameRequest) -> dict:
    ...
    cleaned_name = clean_name(request.name)
    requested_player_id = (request.player_id or "").strip()
    existing_player = room.players.get(requested_player_id) if requested_player_id else None
    if existing_player is None:
        requested_key = name_key(cleaned_name)
        matching_players = [
            player
            for player in room.players.values()
            if name_key(player.name) == requested_key
        ]
        active_name_holder = next(
            (
                player
                for player in matching_players
                if player.connected and player_connection_status(player) == "active"
            ),
            None,
        )
        if active_name_holder is not None:
            raise HTTPException(status_code=400, detail="Igrac sa tim imenom je vec u sobi.")
        existing_player = matching_players[0] if matching_players else None

    if existing_player is not None:
        existing_player.name = cleaned_name
        existing_player.connected = True
        existing_player.last_seen_at = time.time()
        existing_player.connection_state = "active"
        existing_player.disconnect_started_at = None
        existing_player.disconnected_at = None
        existing_player.likely_tab_closed_at = None
        existing_player.play_mode = room.play_mode
        if request.avatar in ALLOWED_AVATARS:
            existing_player.avatar = request.avatar
        ensure_scoreboard_player(room, existing_player.id)
        cancel_empty_room_cleanup(room)
        set_room_event(room, "reconnect", existing_player)
        persist_room(room)
        await broadcast_room(room)
        spectator = room.state != "lobby" and existing_player.id not in room.round_player_ids
        return {
            "room_code": room.code,
            "player_id": existing_player.id,
            "avatar": existing_player.avatar,
            "room_play_mode": room.play_mode,
            "spectator": spectator,
            "message": "Ušao si kao posmatrač. Igraš od sledeće runde." if spectator else "",
        }

    if len(room.players) >= MAX_PLAYERS:
        raise HTTPException(status_code=400, detail="Soba je puna. Maksimalno 8 igraca.")
...
async def start_round(room_code: str, action: StartRoundAction) -> dict:
    ...
    active_players = active_playable_players_for_next_round(room)
...
def advance_to_next_player(room: Room) -> None:
    ...
    clear_live_target_reactions(room)
...
async def update_play_mode(room_code: str, action: PlayModeAction) -> dict:
    ...
    requested_mode = normalize_play_mode(action.play_mode)
    if requested_mode != room.play_mode:
        locked_room_label = "Chat" if room.play_mode == "chat" else "Live"
        raise HTTPException(status_code=400, detail=f"Host je napravio {locked_room_label} sobu.")
    player.play_mode = room.play_mode
    persist_room(room)
    await broadcast_room(room)
    return {"ok": True}
...
async def send_reaction(room_code: str, action: ReactionAction) -> dict:
    ...
    try:
        target_type, target_id = resolve_reaction_target(action)
    ...
    now = time.time()
    identity = reaction_identity(action.player_id, target_type, target_id, emoji)
    repulsed = upsert_active_target_reaction(
        room,
        identity=identity,
    ...
    persist_room(room)
    await broadcast_room(room)
    return {"ok": True, "repulsed": repulsed}
...
async def new_round(room_code: str, action: PlayerAction) -> dict:
    ...
    active_players = active_playable_players_for_next_round(room)
    if len(active_players) < MIN_PLAYERS:
        raise HTTPException(status_code=400, detail="Potrebna su najmanje 4 aktivna igraca za novu rundu.")
...
def room_to_snapshot(room: Room) -> dict:
    return {
        ...
        "active_target_reactions": room.active_target_reactions,
        "turn_version": room.turn_version,
        "players": [
            {
                "id": player.id,
...
def snapshot_play_mode(snapshot: dict) -> str:
    players = snapshot.get("players", [])
    host_id = snapshot.get("host_id")
    for player_data in players:
        if host_id and player_data.get("id") == host_id:
            return normalize_play_mode(player_data.get("play_mode"))
    if players:
        return normalize_play_mode(players[0].get("play_mode"))
    return "live"
...
def room_from_snapshot(snapshot: dict) -> Room:
    ...
    room = Room(
        ...
        host_id=snapshot.get("host_id"),
        play_mode=normalize_play_mode(snapshot.get("play_mode") or snapshot_play_mode(snapshot)),
        ...
        player_list_reactions=normalize_player_list_reactions(snapshot.get("player_list_reactions")),
        active_target_reactions=normalize_active_target_reactions(
            snapshot.get("active_target_reactions"),
            snapshot.get("active_target_reaction"),
        ),
...
def is_player_active_playable_now(player: Player) -> bool:
    return player.connected and player_connection_status(player) == "active"


def active_playable_players_for_next_round(room: Room) -> list[Player]:
    cleanup_expired_disconnected_players(room)
    return [
        player
        for player in room.players.values()
        if player.id not in room.kicked_player_ids and is_player_active_playable_now(player)
    ]
...
def remove_player_from_room(room: Room, player_id: str) -> None:
    ...
    room.player_list_reactions = [
        ...
        and reaction.get("sender_player_id") != player_id
    ]
    room.active_target_reactions = [
        reaction
        for reaction in room.active_target_reactions
        if reaction.get("sender_player_id") != player_id
        and reaction.get("subject_player_id") != player_id
        and reaction.get("target_id") != player_id
    ]
...
def resolve_reaction_target(action: ReactionAction) -> tuple[str, str]:
    target_type = (action.target_type or "").strip()
    target_id = (action.target_id or "").strip()
    if target_type and target_id:
        if target_type not in {"live_player", "chat_association"}:
            raise ValueError("Nepoznat tip reakcije.")
        return target_type, target_id
    legacy_target = (action.target_player_id or "").strip()
    if legacy_target:
        return "live_player", legacy_target
    raise ValueError("Nedostaje meta reakcije.")


def reaction_identity(sender_player_id: str, target_type: str, target_id: str, emoji: str) -> str:
    return f"{sender_player_id}:{target_type}:{target_id}:{emoji}"


def normalize_player_list_reactions(raw: object) -> list[dict]:
...
def normalize_active_target_reactions(raw: object, legacy_single: object) -> list[dict]:
    if isinstance(raw, list):
        return [item for item in raw if isinstance(item, dict)]
    if isinstance(raw, dict):
        return [raw]
...
def clear_all_reactions(room: Room) -> None:
    ...
    room.active_target_reactions.clear()


def clear_live_target_reactions(room: Room) -> None:
    room.active_target_reactions = [
        reaction
        for reaction in room.active_target_reactions
        if reaction.get("target_type") != "live_player"
    ]


def upsert_player_list_reaction(
    room: Room,
    *,
    identity: str,
    ...
    for reaction in room.player_list_reactions:
        if reaction.get("identity") == identity:
            reaction["expires_at"] = now + PLAYER_LIST_REACTION_SECONDS
            reaction["created_at"] = now
            return
    room.player_list_reactions.append(
        {
            "identity": identity,
            "subject_player_id": subject_player_id,
    ...
    trim_player_list_reactions(room)


def upsert_active_target_reaction(
    room: Room,
    *,
    identity: str,
    target_type: str,
    ...
    for reaction in room.active_target_reactions:
        if reaction.get("identity") == identity:
            reaction["repulse_at"] = now
            return True
    room.active_target_reactions.append(
        {
            "identity": identity,
            "target_type": target_type,
            "target_id": target_id,
    ...
    trim_active_target_reactions(room, target_type, target_id)
    return False


def trim_player_list_reactions(room: Room) -> None:
    now = time.time()
    room.player_list_reactions = [
        reaction
        for reaction in room.player_list_reactions
        if reaction.get("expires_at", 0) > now
    ]


def trim_active_target_reactions(room: Room, target_type: str, target_id: str) -> None:
    matching = [
        reaction
        for reaction in room.active_target_reactions
        if reaction.get("target_type") == target_type and reaction.get("target_id") == target_id
    ]
    if len(matching) <= MAX_REACTIONS_PER_TARGET:
        return
    matching.sort(key=lambda reaction: reaction.get("created_at", 0))
    drop_identities = {reaction.get("identity") for reaction in matching[:-MAX_REACTIONS_PER_TARGET]}
    room.active_target_reactions = [
        reaction
        for reaction in room.active_target_reactions
        if reaction.get("identity") not in drop_identities
    ]
...
def cleanup_reactions(room: Room) -> None:
    ...
    kept: list[dict] = []
    for reaction in room.active_target_reactions:
        target_type = reaction.get("target_type")
        target_id = reaction.get("target_id")
    ...
    room.active_target_reactions = kept


def reactions_for_player(room: Room, player_id: str) -> list[dict]:
...
def reaction_for_player(room: Room, player_id: str) -> dict | None:
    reactions = reactions_for_player(room, player_id)
    if not reactions:
        return None
    return reactions[-1]


def active_target_reactions_state(room: Room) -> list[dict]:
    cleanup_reactions(room)
    return [
        {
            "identity": reaction.get("identity"),
...
def active_target_reaction_state(room: Room) -> dict | None:
    reactions = active_target_reactions_state(room)
    if not reactions:
        return None
    return reactions[-1]
...
def public_state(room: Room, viewer_id: str) -> dict:
    ...
    viewer_is_spectator = room.state != "lobby" and viewer_id in room.players and viewer_id not in room.round_player_ids
    spectator_message = "Ušao si kao posmatrač. Igraš od sledeće runde." if viewer_is_spectator else ""
    return {
        "room_code": room.code,
        "state": room.state,
        "viewer_id": viewer_id,
        ...
        "active_playable_player_count": len(active_playable_players_for_next_round(room)),
        ...
        "viewer_is_spectator": viewer_is_spectator,
        ...
        "active_target_reactions": active_target_reactions_state(room),
